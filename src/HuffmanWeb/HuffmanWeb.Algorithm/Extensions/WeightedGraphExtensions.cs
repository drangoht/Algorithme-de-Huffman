    ...
    public static class WeightedGraphExtensions
    {
        public static void ComputeDescendants(this WeightedGraph graph)
        {
            ...
            foreach (var link in links)
            {
                ...
                FillDescendantsCount(graph, nodeParent);

            }
        }
        private static void FillDescendantsCount(WeightedGraph graph, HuffmanNode? parentNode)
        {
            ...
            FillDescendantsCount(graph, graph.Links.FirstOrDefault(n => n.Child == parentNode)?.Parent);
        }
    }
}
